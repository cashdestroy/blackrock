const STORAGE_KEY = "tagtracker_v1";
...
function handleAuth(action) {
  ...
  if (action === "login") {
    if (!existing || existing.password !== password) return notify("Invalid login.");
...
function onTagSubmit(event) {
  ...
  if (!user) return notify("Log in to upload tags.");
...
function onPostSubmit(event) {
  event.preventDefault();
  const user = currentUser();
  if (!user) return notify("Log in to create posts.");
...
function renderSession() {
  ...
  els.sessionView.innerHTML = `
    <p><strong>${escapeHtml(user.username)}</strong> is logged in.</p>
...
function renderTags() {
  ...
  filtered.forEach((tag) => {
    ...
    favBtn.addEventListener("click", () => {
      const user = currentUser();
      if (!user) return notify("Log in to favorite tags.");
      if (isFav) {
...
function renderPosts() {
  ...
  state.posts.forEach((post) => {
    ...
    likeBtn.addEventListener("click", () => {
      const user = currentUser();
      if (!user) return notify("Log in to like posts.");
    ...
    commentForm.addEventListener("submit", (event) => {
      event.preventDefault();
      const user = currentUser();
      if (!user) return notify("Log in to comment.");
