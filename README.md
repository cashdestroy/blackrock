...
- If proxy autofill is blocked, use **Fallback: paste Depop page HTML/text** (or a direct product URL) and click **Parse pasted content**.
...
- The app now also detects `Forbidden`/`Access denied` style responses and clears target fields before each autofill attempt to avoid stale/garbage values.
